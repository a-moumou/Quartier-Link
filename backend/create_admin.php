...
if ($existing) {
    ...
    echo "Voulez-vous promouvoir ce compte en admin ? (o/n) : ";
    $choice = strtolower(trim(fgets(STDIN)));
    if ($choice === 'o') {
        $existing->setRole('ADMIN_QUARTIER');
        $existing->setStatutVerification(VerificationStatus::VERIFIE);
        $em->flush();
        echo "✅ Compte promu administrateur avec succès.\n\n";
    }
    exit(0);
}
...
echo "   Email : {$email}\n";
echo "   Rôle  : ADMIN_QUARTIER (ROLE_ADMIN_QUARTIER + ROLE_USER)\n\n";
